...
public class HikingTrails1 {
    ...
    public void detectTrailScores(){
        for(MapPoint mp : map){
            if(mp.isTrailhead()){
                detectPathScore(mp, mp);
            }
        }
    }

    private void detectPathScore(MapPoint mp, MapPoint trailhead){
        if(mp.isEndpoint()){
            if(mp.getReachedByTrailhead() != trailhead){
                mp.setReachedByTrailhead(trailhead);
                result++;
            }
            return;
        }
        for(Vector dir : directions){
            ...
            if(isInsideMap(neighbourPos)) {
                ...
                if (neighbour.getValue() == mp.getValue() + 1) {
                    detectPathScore(neighbour, trailhead);
                }
            }
        }
    }
